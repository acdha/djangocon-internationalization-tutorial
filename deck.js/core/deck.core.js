  ...
  var slides, // Array of all the uh, slides...
	...
	*/
    updateStates = function() {
      ...
      $all.removeClass(
        [oc.before, oc.previous, oc.current, oc.next, oc.after].join(" ")
      );
      ...
      init: function(elements, opts) {
        ...
        $d.unbind("keydown.deck")
          .bind("keydown.deck", function(e) {
            if (
              e.which === options.keys.next ||
              $.inArray(e.which, options.keys.next) > -1
            ) {
              methods.next();
              e.preventDefault();
            } else if (
              e.which === options.keys.previous ||
          ...
          .undelegate(
            "input, textarea, select, button, meter, progress, [contentEditable]",
            "keydown",
            esp
          )
          .delegate(
          ...
          .bind("touchmove.deck", function(e) {
            $.each(e.originalEvent.changedTouches, function(i, t) {
              if (startTouch && t.identifier === startTouch.identifier) {
                if (
                  t.screenX - startTouch.screenX > tolerance ||
                  t.screenY - startTouch.screenY > tolerance
                ) {
                  $[deck]("prev");
                  startTouch = undefined;
                } else if (
                  t.screenX - startTouch.screenX < -1 * tolerance ||
                  t.screenY - startTouch.screenY < -1 * tolerance
                ) {
                  $[deck]("next");
                  startTouch = undefined;
                }
        ...
        $.each(slides, function(i, $el) {
          $el
            .unbind("webkitTransitionEnd.deck")
            .bind("webkitTransitionEnd.deck", function(event) {
